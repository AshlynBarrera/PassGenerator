...
function randomIndex(str){
  ...
  function getRandomLower(){
    ...
    // Returning a random letter using a random index
  ...
  // Object that store all the character generator functions
  ...
  function generatePassword(lower, upper, number, symbol, length){
    ...
    // NOTE: These values set to the typesCount variable will be used when building out the password
    const typesCount = lower + upper + number + symbol;
    console.log(typesCount);
  
    // If the user has NOT selected any of the four options, then the alert will be displayed and an empty string be returned so the password displayed to the user will be an empty string (AKA Nothing)
    if (typesCount === 0){
    ...
    // Creating an array of arrays. The first item in each nested array holds the value of a string that will be used to access a function in the randomFunctions object. Also, the second item in each nested array are the values passed into this generatePassword function.
    let typesArr = [
      [`lower`, lower],
      [`upper`, upper],
      [`number`, number],
    ...
    // The filter method creates a new array will all the times that "pass the test" implemented by the provided function (AKA All the items that cause the function to return a TRUE value when the function is run  using the item as the argument for the item parameter in this example)
    ...
    for (i = 0; i < length; i += typesCount){
      ...
      // One of items in the updated/filtered version of the typeArr array will be the value/argument passed in for the type parameter each time the anonymous arrow function is run
